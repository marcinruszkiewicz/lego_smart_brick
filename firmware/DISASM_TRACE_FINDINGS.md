...
The AES-128-CCM encryption key for NFC tag data is **NOT stored in the firmware binary**. It resides in the hardware key register at `0x808904`, which is a secure storage area in the MCU (likely EM9305's OTP or key slot memory). The key is loaded into the AES crypto engine at runtime by reading this register.
...
## Implications for Key Recovery

1. **The key cannot be extracted from the firmware binary.** It's in hardware.
2. **Options for key recovery:**
   - Read register `0x808904` from a running device (requires debug access / JTAG)
   - Intercept key provisioning from the Bilbo Enigma API (`CreateSpkitBundle`, `SharedKey`)
   - Use mitmproxy to capture SmartAssist ↔ Bilbo traffic during tag scanning
   - If the key is per-device: dump it from the brick's MCU via debug interface
   - If the key is global: obtain from any single device

3. **AES-CCM parameters (still to be determined):**
   - Key size: 16 bytes (AES-128) — confirmed by `mov_s r3, 0x10`
   - Nonce length: unknown (7-13 bytes)
   - MAC length: unknown (4/8/16 bytes)
   - AAD: unknown (empty, header, or UID)
   - These parameters might be determinable from the crypto engine register layout
